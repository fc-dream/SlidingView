...
class PositionHelper {
     ...
     * 猜测想要到达的位置
     *
     * @param endX   起始位置的x轴坐标
     * @param startY 起始位置的y轴坐标
    ...
    /**
     * 猜测想要达的位置
     * <p>算法首先根据起始位置和当前位置，根据{@link #guessPosition(Position, Position)}猜测一个想要
     * 到达的目标位置p。然后判断滑动的速度是否到达一个阈值，如果是则想要到达的位置就为p。 如果速度没
     * 有达到阈值，判断已经移动的距离是否已经占总距离的一半，如果是，则想要到达的位置就是p。否则目标
     * 位置就是移动前的位置</p>
     *
     * @param velocityX x轴的滑动速度
     * @param velocityY y轴的滑动速度
     * @param startX    起始位置的x轴坐标
     * @param startY    起始位置的y轴坐标
     * @param endX      当前位置的x轴坐标
     * @param endY      当前位置的y轴坐标
     * @return 想要到达的目标位置所对应的Id
     */
